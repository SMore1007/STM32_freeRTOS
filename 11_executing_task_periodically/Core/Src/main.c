...
TaskHandle_t greenTaskHandle, blueTaskHandle, redTaskHandle;   // Task Handles
...
void blueLEDControllerTask(void *pvParameters)
{
	...
	const TickType_t _500msPeriod = pdMS_TO_TICKS(500);
	xLastWakeTime = xTaskGetTickCount();                // last running time

	while(1)
	{
		vTaskDelayUntil(&xLastWakeTime, _500msPeriod);
		BlueLedCounter++;
